...
/*    if (!ble_rn4870.enterConfigurationMode()) {
        SerialUSB.print("Cannot enter Configuration mode");
        while(1){};
    }

            if (ble_rn4870.setName("SmartShit", 9)) {
                SerialUSB.print("Name changed correctly");
            } else {
                SerialUSB.println("Error in setName ");
            }


    if (!ble_rn4870.enterDataMode()) {
        SerialUSB.print("Cannot enter Data mode");
        while(1){};
    }
*/

char ra[6] = {0x01,0x02,0x03,0x04,0x05,0x06 };
char *name = "SmartShit";

    if (!ble_rn4870.startBLE(ra,name)) {
